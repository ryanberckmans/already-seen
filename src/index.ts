...
//   Add master branch protection and a tampermonkey dynamic release to test current branch; should include versioning the tampermonkey script based on HEAD commit id, because tampermonkey seems to have trouble updating a script if the version doesn't change; or maybe a way to install tampermonkey from local update url (file:// or local webserver?), so that you could test tampermonkey without having to push to github
